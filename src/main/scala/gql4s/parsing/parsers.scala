...
val fragmentSpread = (string("...") ~ __ *> (fragmentName <* __) ~ directives0).map { case name -> directives =>
...
val rootOperationTypeDefinitions =
    ((char('{') ~ __).with1 *> (rootOperationTypeDefinition <* __).rep <* char('}')).map(_.toList)
...
val schemaExtensionA = ((directives0 <* __) ~ rootOperationTypeDefinitions).map { case dirs -> root =>
    SchemaExtension(dirs, root.toList)
}
...
val scalarTypeDefinition = (desc ~ scalar *> (name <* __) ~ directives0).map { case name -> directives =>
...
val implementsInterfaces = recursive[List[NamedType]] { recurse =>
...
val objectTypeExtensionB =
    ((name <* __) ~ (implementsInterfaces0 <* __) ~ directives <* !char('{')).map { case name -> impls -> dirs =>
        ObjectTypeExtension(name, impls, dirs.toList, Nil)
    }
...
val interfaceTypeExtensionB =
    ((name <* __) ~ (implementsInterfaces0 <* __) ~ directives <* !char('{')).map { case name -> impls -> dirs =>
        InterfaceTypeExtension(name, impls, dirs.toList, Nil)
    }
val interfaceTypeExtensionC = ((name <* __) ~ (implementsInterfaces <* __) <* !char('{')).map { case name -> impls =>
    InterfaceTypeExtension(name, impls.toList, Nil, Nil)
}
...
val unionMemberTypes =
    ((__ ~ (char('|') ~ __).?) *> (namedType <* __).? ~
        recursive[List[NamedType]] { recurse =>
...
val unionTypeExtensionA =
    ((name <* __) ~ (directives0 <* (__ ~ char('='))) ~ unionMemberTypes).map { case name -> dirs -> unionMembers =>
        UnionTypeExtension(name, dirs, unionMembers.toList)
    }
val unionTypeExtensionB = ((name <* __) ~ (directives <* __ ~ char('=') ~ __)).map { case name -> dirs =>
    UnionTypeExtension(name, dirs.toList, Nil)
}
...
val enumTypeExtensionB = ((name <* __) ~ (directives <* __) <* !char('{')).map { case name -> dirs =>
    EnumTypeExtension(name, dirs.toList, Nil)
}
...
val inputObjectTypeExtensionB = ((name <* __) ~ (directives <* __) <* !char('{')).map { case name -> dirs =>
    InputObjectTypeExtension(name, dirs.toList, Nil)
}
...
val executableDocument   = __ *> (executableDefinition <* __).rep.map(_.toList).map(ExecutableDocument.apply)
...
val typeSystemDocument              = __ *> (typeSystemDefinition <* __).rep.map(_.toList).map(TypeSystemDocument.apply)
